...
class ActionButtonsWindow(QWidget):
    """
    A separate window containing the main action buttons for calculations.
    """
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Alprotein Actions")
    ...
    def setup_ui(self):
        ...
        group_box.setStyleSheet("QGroupBox { color: black; font-weight: bold; }")
        ...
        # Info label
        self.info_label = QLabel("💡 Site energies must be calculated before CDC analysis")
    ...
    def set_site_energies_calculated(self, calculated: bool):
        ...
        self.update_button_states()
        if calculated:
            self.info_label.setText("✅ Site energies calculated - CDC analysis ready")
        else:
            self.info_label.setText("💡 Site energies must be calculated before CDC analysis")
