...
# spectra_new_data = '/Users/mohamed/Documents/Research/Projects/SuperComplex_Kajwal/correct_calculation/PSII_exp_spectra/CP24/Urlich_2024/'

# Configuration
PDB_FILE_PATH = "/Users/mohamed/Documents/Research/Projects/SuperComplex_Kajwal/correct_calculation/CP24/CP24_LYS_gold/fit_HIS_183/most_occ/most_occ_pH8.pdb"
EXP_DATA_BASE_PATH = '/Users/mohamed/Documents/Research/Projects/SuperComplex_Kajwal/correct_calculation/PSII_exp_spectra/CP24/Urlich_2024/'
PATH_EXP_ABS = f'{EXP_DATA_BASE_PATH}A_CP24_urlich_nm_300K.npy'
PATH_EXP_FLUO = f'{EXP_DATA_BASE_PATH}F_CP24_urlich_nm_300K.npy'
...
RESULTS_DIR = Path(__file__).parent / "results_cp24"
...
try:
    ...
    protein_structure = ProteinStructure.from_file_extended(str(pdb_path), "test_protein")
...
try:
    site_energy_calculator = SiteEnergyCalculator(
        ...
        e0a=14550.0,  # Fine-tuned to match experimental peak at 675 nm
        e0b=15000.0   # Fine-tuned to match experimental peak at 675 nm
    )
    ...
    print(f"    ✗ Error calculating site energies: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    hamiltonian_calculator = HamiltonianCalculator(
        ...
        f_val=0.72,
        E_0a=14550.0,  # Fine-tuned to match experimental peak at 675 nm
        E_0b=15000.0   # Fine-tuned to match experimental peak at 675 nm
    )

    # Use dipole coupling by default
    ...
    print(f"    ✗ Error constructing Hamiltonian: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    print(f"    ✗ Error defining domains: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

# ============================================================================
# STEP 5: Calculate Spectrum with Proper Broadening
# ============================================================================
print("\n[5] Calculating absorption spectrum...")

try:
    # === Choose calculation method ===
    use_proper_spectra_calculator = False  # Set to False for simple Gaussian approach

    if use_proper_spectra_calculator:
        # === Use Proper SpectraCalculator with Renger Lineshape Theory ===
        print("    Using SpectraCalculator with phonon-exciton coupling and proper broadening...")

        # Parameters
        temperature = 300.0  # K
        disorder_fwhm = 300.0  # cm⁻¹ - FWHM for inhomogeneous broadening (diagonal disorder)
        n_ensemble = 100  # Number of disorder realizations (reduced for speed)
        dt = 1.0  # Time step in fs
        t_max = 1000.0  # Maximum time in fs
        ...
        # Calculate spectrum
        wavelengths, spectrum, A_00, A_01 = spectra_calc.calculate_spectrum(
        ...
        # Find peak
        # Filter to relevant range for peak finding
        mask = (wavelengths >= 600) & (wavelengths <= 720)
        if np.any(mask):
            peak_idx_masked = np.argmax(spectrum[mask])
            wavelengths_masked = wavelengths[mask]
            peak_wl = wavelengths_masked[peak_idx_masked]
        else:
...
try:
    ...
    ax1.plot(wavelengths, spectrum, 'b-', linewidth=2.5, label='Simulated')
    ax1.set_xlabel('Wavelength (nm)', fontsize=12)
    ax1.set_ylabel('Absorption (normalized)', fontsize=12)
    ax1.set_title(f'CP24 Absorption Spectrum (Peak: {peak_wl:.1f} nm)', fontsize=14)
    ...
    # === FIGURE 2: Site Energy Shifts ===
    fig2, ax2 = plt.subplots(figsize=(12, 6))

    # Calculate site energy shifts (relative to E0a)
    E0a = 14550.0  # vacuum energy for CLA (fine-tuned to match calculator)
    ...
    bars = ax2.bar(x_pos, shifts, color=colors, alpha=0.8, edgecolor='black', linewidth=0.5)
    ...
    ax2.set_xlabel('Pigment (CLA residue number)', fontsize=12)
    ax2.set_ylabel(f'Site Energy Shift (cm⁻¹) relative to E₀ = {E0a} cm⁻¹', fontsize=12)
    ax2.set_title('Site Energy Shifts from CDC Calculation', fontsize=14)
    ...
    output_file2 = RESULTS_DIR / "site_energy_shifts.png"
    fig2.savefig(output_file2, dpi=150, bbox_inches='tight')
    print(f"    ✓ Saved site energy shift plot to: {output_file2}")
